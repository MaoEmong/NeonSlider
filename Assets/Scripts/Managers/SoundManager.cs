...
public class SoundManager
{
	...
	public void Init()
	{
		...
		if(root == null)
		{
			...
			for(int i = 0; i < soundNames.Length - 1; i++)
			{
                ...
                _audioSource[i].dopplerLevel = 0.0f;
                _audioSource[i].reverbZoneMix = 0.0f;
			}
		}
        // Bgm은 반복설정
    ...
    {
        Managers.GData.Options.BGMVol = vol;
        _audioSource[(int)Define.Sound.Bgm].volume = Managers.GData.Options.BGMVol;
    }
    public float GetBGMVol()
    {
        return Managers.GData.Options.BGMVol;
    }
    public void SetEffectVol(float vol) 
    {
		Managers.GData.Options.SEVol = vol;
		_audioSource[(int)Define.Sound.Effect].volume = Managers.GData.Options.SEVol * 0.8f;
    }
    public float GetEffectVol()
    {
        return Managers.GData.Options.SEVol;
    }
